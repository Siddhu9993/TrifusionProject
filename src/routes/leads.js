...
router.post('/', contactLimiter, async (req, res) => {
    try {
        ...
        // TODO: send email notification (requires RESEND_API_KEY or SMTP config)
        // await sendLeadNotification(lead);
        // await sendLeadAcknowledgement(lead);
...
router.get('/:id', auth_js_1.authenticateToken, auth_js_1.requireAdmin, async (req, res) => {
    try {
        const lead = await prisma_js_1.prisma.lead.findUnique({ where: { id: req.params['id'] }, include: { notes: { include: { user: { select: { name: true, email: true } } } } } });
        if (!lead)
...
router.patch('/:id/status', auth_js_1.authenticateToken, auth_js_1.requireAdmin, async (req, res) => {
    try {
        ...
        const lead = await prisma_js_1.prisma.lead.update({ where: { id: req.params['id'] }, data: { status } });
        return (0, response_js_1.ok)(res, lead);
    }
    catch (err) {
        return (0, response_js_1.serverError)(res, err);
...
router.post('/:id/notes', auth_js_1.authenticateToken, auth_js_1.requireAdmin, async (req, res) => {
    try {
        ...
        const newNote = await prisma_js_1.prisma.leadNote.create({
            data: { leadId: req.params['id'], userId: authReq.user.id, note: note.trim() },
        });
...
router.delete('/:id', auth_js_1.authenticateToken, auth_js_1.requireAdmin, async (req, res) => {
    try {
        await prisma_js_1.prisma.lead.delete({ where: { id: req.params['id'] } });
