...
import { ok, created, fail, serverError, getPagination } from '../utils/response.js';
...
const leadSchema = z.object({
    ...
    preferredContactMethod:  z.string().max(100).optional(),
...
router.post('/', contactLimiter, async (req, res) => {
    try {
        ...
        if (!parsed.success) {
            return fail(res, parsed.error.issues[0]?.message || 'Validation error');
        }

        const { name, email, phone, company, serviceInterest, budgetRange, preferredContactMethod, message } = parsed.data;

        // Generate a unique submission ID
        const id        = `TF-LEAD-${uuidv4().replace(/-/g, '').slice(0, 8).toUpperCase()}`;
...
router.patch('/:id/status', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        if (!updated) return fail(res, 'Lead not found', 404, 'NOT_FOUND');

        return ok(res, { id, status, message: 'Status updated' });
    } catch (err) {
        return serverError(res, err);
    }
});
...
router.get('/test-connection', authenticateToken, requireAdmin, async (_req, res) => {
